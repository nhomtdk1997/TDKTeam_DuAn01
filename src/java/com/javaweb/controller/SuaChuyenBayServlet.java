 ...
 * @author DuongNguyen
 */
public class SuaChuyenBayServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String Diemkhoihanh, Diemden, giocatcanh, giohacanh, Loaimaybay, ngaykhoihanh, idChuyenbay;

        idChuyenbay = request.getParameter("idCB");
        ...
        Chuyenbay cb = cbsv.GetChuyenbayById(idChuyenbay);

        cb.setTu(Diemkhoihanh);
        cb.setDen(Diemden);
        cb.setNgayKhoiHanh(Ngaykhoihanh);
        cb.setGioCatCanh(Giocatcanh);
        cb.setGioHaCanh(Giohacanh);
        cb.setLoaiMayBay(Loaimaybay);
        cb.setGiaChuyenBay(Giachuyenbay);
        
        HttpSession session = request.getSession();
        boolean kt = cbsv.InsertChuyenbay(cb);
        if (kt) {
            session.setAttribute("kiemtra", "1");
            response.sendRedirect("SuaChuyenBay.jsp?idChuyenbay=" + idChuyenbay);
        } else {
...
//            out.println("<html>");
//            out.println("<head>");
//            out.println("<title>Servlet SuaChuyenBayServlet</title>");            
//            out.println("</head>");
//            out.println("<body>");
//            out.println("<h1>Servlet SuaChuyenBayServlet at " + request.getContextPath() + "</h1>");
//            out.println("</body>");
//            out.println("</html>");
//        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(SuaChuyenBayServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
